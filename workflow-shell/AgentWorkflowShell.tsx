...
import {Activity, ArrowRight, Bot, CheckCircle2, Chrome, Database, Play, RefreshCcw, ShieldCheck, Workflow} from 'lucide-react';
import {APP_WORKFLOW_ADAPTERS, getAppWorkflowAdapter} from './appAdapters';
import {getVideoAgentRuntimeStatus, queueAppWorkflow} from './bridgeClient';
...
export default function AgentWorkflowShell({onBack}:{onBack?:()=>void}) {
  ...
  async function refreshStatus(){
    setError('');
    try {
      const result=await getVideoAgentRuntimeStatus();
      setRuntime(result);
    } catch (err:any) {
  ...
  async function run(kind:string){
    ...
    try {
      const result=await queueAppWorkflow({appId:app.appId,projectId:app.projectId,kind,templateCandidates:app.templates,qa:true,writeback:true});
  ...
  return <main className="min-h-screen bg-[#050811] text-white">
    <header className="border-b border-white/10 bg-[#070b14] px-5 py-4">
      <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          {onBack&&<button onClick={onBack} className="rounded-xl border border-white/15 px-3 py-2">Back</button>}
          <button onClick={refreshStatus} disabled={busy} className="rounded-xl border border-white/15 px-3 py-2 disabled:opacity-50"><span className="flex items-center gap-2"><RefreshCcw size={15}/> 상태확인</span></button>
          <button onClick={()=>run('FULL_E2E')} disabled={busy} className="rounded-xl bg-emerald-400 px-4 py-2 font-black text-black disabled:opacity-50"><span className="flex items-center gap-2"><Play size={16}/>{busy?'실행 중':'E2E 실행'}</span></button>
        </div>
      </div>
    </header>
    ...
    <section className="mx-auto grid max-w-[1500px] gap-4 p-5 xl:grid-cols-[280px_1fr]">
      ...
      <div className="space-y-4">
        {(error||lastResult||runtime)&&<section className={`rounded-2xl border p-4 ${error?'border-red-500/30 bg-red-500/10':'border-emerald-500/20 bg-emerald-500/[0.06]'}`}>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-xs font-black text-slate-400">RUNTIME READBACK</div><div className={`mt-1 font-black ${error?'text-red-300':'text-emerald-300'}`}>{error|| (runtimeOk?'BRIDGE READY':'BRIDGE RESPONSE')}</div></div><div className="text-xs text-slate-500">{runtime?.version||runtime?.data?.version||''}</div></div>
          {lastResult&&<div className="mt-2 text-xs text-slate-300">최근 실행: {lastResult.message||lastResult.action||'QUEUED'} {lastResult?.data?.taskId?`· ${lastResult.data.taskId}`:''}</div>}
        ...
        <section className="rounded-3xl border border-white/10 bg-[#080d18] p-5"><div className="font-black">Template Router</div><div className="mt-4 flex flex-wrap gap-2">{app.templates.map(x=><button key={x} disabled={busy} onClick={()=>run('TEMPLATE:'+x)} className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-200 disabled:opacity-50">{x}</button>)}</div><p className="mt-4 text-xs text-slate-500">각 버튼은 해당 앱의 실제 백데이터/프런트 계약을 사용해 전용 Video Agent로 작업을 넘기고 QA/배포/학습 writeback까지 연결하는 계약입니다.</p></section>
      </div>
    </section>
  </main>;
}
